...
capital_corriente_dy <- 
  ...
  dySeries(fillGraph = FALSE, "GASTOS CORRIENTES (REALIDAD)", 
           label = "Gasto Corriente (Realidad)", color="red") %!>% 
  dySeries(fillGraph = FALSE, "GASTOS DE CAPITAL (REALIDAD)", 
...
capital_dy <- 
  ...
  dySeries(fillGraph = FALSE, "GASTOS DE CAPITAL (REALIDAD)", 
           label = "Realidad", color="red") %!>% 
  dySeries(fillGraph = FALSE, "GASTOS DE CAPITAL (PRESUPUESTO)", 
           label = "Presupuesto", color="green") %!>% 
  dyLegend(width = 300, show = "onmouseover", labelsSeparateLines = TRUE) %!>% 
  dyOptions(stackedGraph = FALSE, axisLineColor = "#98918B") %!>% 
  dyAxis("x", drawGrid = FALSE) %>%
